...
把景点与天气交给高德，换来的是可验证性，代价是**额度**。高德自 2025-05-20 起取消日配额、改为按「服务组」共享的月配额：同一服务组内所有接口、同一账号下所有 Key（含前端 JS API Key）合计消耗同一份额度，个人认证开发者的基础搜索服务只有 5,000/月。而一次 5 天单城市规划就要消耗 13 次搜索（3 次景点 + 10 次餐饮周边）——**满打满算一个月约可规划 384 次**。

这个约束塑造了三处设计：

**分池，而不是一个总额度。** 三个服务组的额度差 30 倍，失败后果也完全不同：搜索池耗尽使规划直接不可用，天气池耗尽只是降级为"按晴天规划"，LBS 池耗尽只影响步行路线绘制。合成一个计数器必然出现"天气额度用完导致规划被拦"这类用户无法理解的结果。

**发出前预占，而不是事后记账。** 事后记账报的是已经发生的超额，阻止不了它。预占让"额度不足"表现为一次请求都不发出。

**按实际请求计数，而不是按逻辑调用。** 一次逻辑搜索在高德层（最多 4 次）× HTTP 层（最多 3 次）的重试下最坏展开成 12 个真实请求，高德按收到的请求扣配额——只有按请求计数才不失真。代价是偏保守（超时的请求也被扣），而偏保守正是这个机制的目的。

配套地，配额类错误从"可重试"里摘了出来：此前 `USER_DAILY_QUERY_OVER_LIMIT` 被当成限流，额度见底时程序反而会加倍索取。

**这套机制是估算，不是对账。** 高德没有配额查询接口，本地计数从部署那刻起从 0 开始，看不到此前的消耗；系统因此提供一个人工对账入口，把控制台读到的数字录进来补正。把估算包装成"剩余额度"，比不做保护更危险。
