...
PLANNER_SYSTEM = (
    ...
    "5.严格遵守抵达与返程时刻：抵达日不排用户尚未到达就无法进行的活动，"
    "返程日的活动必须在返程时刻前留出赶车缓冲结束；prompt 给出具体限制时以它为准。"
...
TIME_CHECK_SYSTEM = (
    ...
    "每天日期与星期判断\n"
    "5. 首日某个景点的 start_time 早于 prompt 给出的『抵达可用下界』——用户那时还没到\n"
    "6. 末日某个景点的 end_time 晚于 prompt 给出的『末日最晚结束时刻』——用户要赶车\n"
    "⚠️ 第 5、6 条只在 prompt 明确给出对应时刻时才适用。prompt 没有给出抵达或返程时刻时，"
    "这两类违规一律不成立，绝不能凭猜测列入。\n"
