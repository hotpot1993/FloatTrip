...
Define how a formal planning run is presented inside the conversation: understandable status vocabulary for every lifecycle state, discrete progress without invented percentages, the waiting-user interaction as the primary action, a result preview on success, recovery actions on failure, a composer that stays usable, and accessible status announcements.
...
本条约束页面呈现；Run 的恢复契约（暴露结构化问题、回答后继续同一个 Run）由 `agent-run-lifecycle` 的 `Human interaction and resume` 定义。
...
后端侧的会话隔离与并发保证见 `planning-concurrency` 的 `Chat remains responsive during planning`。
